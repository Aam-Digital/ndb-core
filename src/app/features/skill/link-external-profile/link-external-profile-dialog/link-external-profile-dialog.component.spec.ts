...
describe("LinkExternalProfileDialogComponent", () => {
  ...
  it("should show error but not throw if API request fails", async () => {
    vi.useFakeTimers();
    try {
  ...
  it("should show error message if API rejects for missing permissions (403)", async () => {
    vi.useFakeTimers();
    try {
      const mockError = new HttpErrorResponse({
        status: 403,
  ...
  it("should show 'no results' error if API returns empty", async () => {
    vi.useFakeTimers();
    try {
      mockSkillApi.getExternalProfiles.mockReturnValue(
        of({
  ...
  it("should automatically select if API returns only a single result", async () => {
    vi.useFakeTimers();
    try {
      const mockResult: ExternalProfile = {
        id: "1",
        fullName: "match 1",
      } as any;
      mockSkillApi.getExternalProfiles.mockReturnValue(
  ...
  it("should add more results when loading next page", async () => {
    vi.useFakeTimers();
    try {
