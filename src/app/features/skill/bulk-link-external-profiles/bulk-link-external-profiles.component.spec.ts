...
describe("BulkLinkExternalProfilesComponent", () => {
  ...
  it("should gracefully skip and continue if request for one record fails", async () => {
    vi.useFakeTimers();
    try {
      const expected2 = {
        entity: component.entities[1],
  ...
  it("should pre-select the possible match if exactly one result", async () => {
    vi.useFakeTimers();
    try {
      mockSkillApi.getExternalProfiles
  ...
  it("should pre-select existing link from record", async () => {
    vi.useFakeTimers();
    try {
  ...
  it("should show warning if existing link from record cannot be loaded", async () => {
    vi.useFakeTimers();
    try {
  ...
  it("should save all changed entities upon confirmation and also apply skills data", async () => {
    vi.useFakeTimers();
    try {
      ...
      const savedEntities = vi.mocked(mockEntityMapper.saveAll).mock
        .lastCall[0];
  ...
  it("should skip record if API throws error and continue with other records without aborting", async () => {
    vi.useFakeTimers();
    try {
      mockSkillApi.applyDataFromExternalProfile.mockImplementation(
