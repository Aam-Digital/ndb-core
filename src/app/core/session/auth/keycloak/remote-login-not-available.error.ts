 ...
 * an OperationTimeoutError) is preserved on `cause` so that callers can
 * distinguish transient infra failures (which are worth retrying) from
 * genuinely unavailable setups (e.g. browser is offline).
 */
