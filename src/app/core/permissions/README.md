...
- **Reserved section keys**: `_default` and `_public` are reserved section keys, not roles. The leading underscore keeps them from colliding with a realm role of the same name, and any user role that starts with `_` is ignored when resolving rules. The non-prefixed `default` / `public` spellings of older documents are no longer read: a stored document is brought to the underscore form by the `oneoff-20260724-permissions-key-rename` migration, and the leftover legacy sections are dropped by `oneoff-20260804-permissions-key-legacy-cleanup` (both in `cli/migration/`). An instance that has not run those migrations loses its shared rules, so they are a precondition for this version. `_default` and `_public` are the only allowed underscore-prefixed keys; do not create realm roles, or any other rule section, whose name starts with `_`.
