...
export class FilterComponent<T extends Entity = Entity> {
  ...
  filterOptionSelected(filter: Filter<T>, selectedOptions: string[]) {
    ...
    if (this.useUrlQueryParams()) {
      this.tableStateUrl.updateFilterParam(
        filter.name,
        selectedOptions.map((value) => encodeURIComponent(value)).join(","),
        false,
      );
    }
  }
   ...
   *
   * Multiple selected values are joined by "," in the URL. Each value is
   * encoded with `encodeURIComponent` before being joined (see
   * `filterOptionSelected`), so a "," inside a value itself is escaped and
   * cannot be mistaken for the multi-value separator.
   */
  private parseFilterValue(raw: string): string[] {
    return raw
      .split(",")
      .filter((value) => value !== "")
  ...
  private loadUrlParams(filterSelections: Filter<T>[]): Filter<T>[] {
    ...
    const updatedSelections = filterSelections.map((filter) => {
      ...
      if (Object.hasOwn(params, filter.name)) {
        nextValues = this.parseFilterValue(params[filter.name]);
      } else if (
        hasUrlParams &&
