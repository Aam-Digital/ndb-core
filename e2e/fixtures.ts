...
import { defaultJsonConfig } from "#src/app/core/config/config-fix.js";
...
export async function loadApp(
  page: Page,
  entities: Iterable<Entity> | undefined = undefined,
) {
  if (entities) {
 ...
 */
function serializeEntities(entities: Iterable<Entity>): unknown[] {
  ...
  entityConfigService.setupEntities(
    Object.entries(config.data).flatMap(([id, config]) => {
