...
test.describe('Dashboard Page Tests', () => {
  ...
  if (await pageIndicator.isVisible()) {
    ...
    const EditStatusBtn = page
      .locator('app-attendance-status-select > .mat-mdc-form-field > .mat-mdc-text-field-wrapper > .mat-mdc-form-field-flex')
      .first();
...
// Click the end date
  ...
  test('Verify attendance percentage color coding', async ({ page }) => {
    ...
    const CoachingAttendance = page.getByRole('columnheader', { name: 'Attendance (School)' }).locator('app-entity-field-label');
  ...
  test('Dashboard View', async ({ page }) => {
    ...
    if (absentCount > 0) {
      ...
      for (let i = 0; i < absentCount; i++) {
        ...
        const daysAbsent = student.locator('app-attendance-day-block > .mat-mdc-tooltip-trigger');
    ...
    if (AbsentCount > 0) {
      ...
      for (let i = 0; i < AbsentCount; i++) {
        ...
        // Get all elements representing days the student was absent
        const daysAbsent = student.locator('app-attendance-week-dashboard:nth-child(8) > app-dashboard-list-widget > app-dashboard-widget > .widget-content > app-widget-content > .table-wrapper > .mat-mdc-table > .mdc-data-table__content > tr > td:nth-child(2) > .activities-record > app-attendance-day-block:nth-child(2) > .mat-mdc-tooltip-trigger').first();
