...
In other words, it is not always suitable to call the entities _Child_ or _School_ and the icon might also be
misleading.
Therefore, the names and icons of entities can be changed through the config.
This allows to change a text like _Participants with unhealthy BMI_ to _Patients with unhealthy BMI_.
...
To allow multiple selection, additionally `dataArray` should be `true`.
